...
px_void PX_Object_CloudRender(px_surface* psurface, PX_Object* pObject, px_dword elpased)
{
	...
	PX_TextureRenderEx(psurface, pdesc->ptextures, (px_int)pObject->x, (px_int)pObject->y, PX_ALIGN_CENTER, PX_NULL, (px_float)psurface->width / 1920 * 0.8, 0);
}
...
PX_Object* PX_Object_CloudCreate(PX_World* pWorld, PX_Runtime* pruntime)
{
	...
	default:
	case 0:
		desc.ptextures = PX_ResourceLibraryGetTexture(&pruntime->ResourceLibrary, "cloud01");
		break;
	case 1:
		desc.ptextures = PX_ResourceLibraryGetTexture(&pruntime->ResourceLibrary, "cloud02");
		break;
	case 2:
		desc.ptextures = PX_ResourceLibraryGetTexture(&pruntime->ResourceLibrary, "cloud03");
		break;
	}
